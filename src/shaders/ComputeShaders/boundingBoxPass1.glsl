...
void initSharedMemory(const uint localId, const uint globalId) {
    ...
    localMin[localId] = globalId < numParticles ? positions[globalId].xyz : vec3(LARGE_FLOAT);
    localMax[localId] = globalId < numParticles ? positions[globalId].xyz : vec3(SMALL_FLOAT);
...
void performReduction(const uint localId, const uint workGroupSize) {
    ...
    for (uint s = workGroupSize >> 1; s > 0; s >>= 1) { // s = BLOCK_SIZE/2, BLOCK_SIZE/4, ..., 1
