...
shared uint sharedBuffer[BLOCK_ELEMENTS];
...
void main() {
    ...
    const uint val1 = getNum(groupId * BLOCK_ELEMENTS + localId);
    const uint val2 = getNum(groupId * BLOCK_ELEMENTS + localId + WG_SIZE);

    sharedBuffer[localId] = val1;
    sharedBuffer[localId + WG_SIZE] = val2;

    groupMemoryBarrier();
    barrier(); // Ensure shared memory writes are complete
    ...
    for (uint depth = 0; depth < log2(BLOCK_ELEMENTS); depth++) {
        ...
        if (writeIndex < BLOCK_ELEMENTS) {
            sharedBuffer[writeIndex] += sharedBuffer[readIndex];
        }
    ...
    if (localId == WG_SIZE - 1) {
        blockSums[groupId] = sharedBuffer[BLOCK_ELEMENTS - 1];
        sharedBuffer[BLOCK_ELEMENTS - 1] = 0; // Reset for the down-sweep phase
    }
    ...
    for (int depth = int(log2(BLOCK_ELEMENTS)) - 1; depth >= 0; depth--) {
        ...
        if (writeIndex < BLOCK_ELEMENTS) {
            uint temp = sharedBuffer[readIndex];
            sharedBuffer[readIndex] = sharedBuffer[writeIndex];
            sharedBuffer[writeIndex] += temp;
        }
    ...
    if (secondPass) {
        blockSums[groupId * BLOCK_ELEMENTS + localId] = sharedBuffer[localId];
        blockSums[groupId * BLOCK_ELEMENTS + localId + WG_SIZE] = sharedBuffer[localId + WG_SIZE];
    }
    else {
        edges[groupId * BLOCK_ELEMENTS + localId] = sharedBuffer[localId];
        edges[groupId * BLOCK_ELEMENTS + localId + WG_SIZE] = sharedBuffer[localId + WG_SIZE];
    }
}
