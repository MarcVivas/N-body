...
parser.add_argument('--algorithms', type=int, nargs='+', choices=[1, 2, 3],
...
algorithms = [1, 2, 3] if args.algorithms is None else args.algorithms
...
for algorithm in algorithms:
    for n in bodies:
        ...
        new_row = pd.DataFrame({'Algorithm': algorithm, 'Bodies': n, 'Elapsed Time': elapsed_time,
                                ...
                                'Iterations': iterations}, index=[0])
        df = pd.concat([df, new_row], ignore_index=True)

algorithm_names = {1: "Sequential Particle-particle CPU", 2: "Parallel Particle-particle CPU", 3: "Parallel Particle-particle GPU"}
